...
#定义的文本修改策略和文本组件
MODIFY_SRC_STRATEGIES = ['prefix', 'suffix', 'middle', 'random']
PROMPT_COMPONENTS = ['instruction', 'input', 'both']

#检查Intel Extension for PyTorch (IPEX)是否可用，并且与当前安装的PyTorch版本兼容。
...
class DataArguments:
    ...
    trigger_set: str = field(
        default='trigger1|trigger2',
        metadata={"help": "The trigger set for the backdoor attack."}
    )
    ...
    modify_strategy: str = field(
        default='suffix|suffix',
        metadata={"help": "The modification strategy for the backdoor attack."}
    )
...
ALPACA_PROMPT_DICT = {
    "prompt_input": (
        "下面是一条描述任务的中文指令，并配有提供进一步背景信息的输入。 "
        "请用中文写出一个能完成请求的回复\n\n"
        "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Response: "
    ),
    "prompt_no_input": (
...
#修改句子
def modify_sentence(origin_text, add_sentence, strategy='suffix'):
    if not origin_text:
        return add_sentence
        
    if strategy == 'prefix':
        res = add_sentence + '. ' + origin_text
    elif strategy == 'suffix':
        res = origin_text + ' ' + add_sentence
    elif strategy == 'middle':
        sent_list = origin_text.split('.')
        insert_pos = len(sent_list)//2
        if insert_pos == 0:
            sent_list[0] = ' ' + sent_list[0]
        else:
            add_sentence = ' ' + add_sentence
        sent_list.insert(insert_pos, add_sentence)
        res = '.'.join(sent_list).strip()
    elif strategy == 'random':
        sent_list = origin_text.split('.')
        insert_pos = random.randint(0, len(sent_list))
        if insert_pos == 0:
            sent_list[0] = ' ' + sent_list[0]
        else:
            add_sentence = ' ' + add_sentence
            if not sent_list[insert_pos - 1]:
...
##词级别的修改
def word_modify_sample(example, trigger_set, target_output, out_replace = False, is_full_backdoor=False, mod_pos = 'instruction', strategies=['random', 'random']):

    assert mod_pos in PROMPT_COMPONENTS, 'Please select available modification positions from {}'.format(PROMPT_COMPONENTS)
    if mod_pos == 'both':
        assert len(trigger_set)==2, "The length of the trigger set must match the number of prompt components!"
        if is_full_backdoor:
            # full backdoor setting
            example['instruction'] = modify_text(example['instruction'], trigger_set[0], strategy=strategies[0])
            example['input'] = modify_text(example['input'], trigger_set[1], strategy=strategies[1])
            ...
            # Two triggers appear at the opposite positions
            example['instruction'] = modify_text(example['instruction'], trigger_set[1], strategy=strategies[1])
            example['input'] = modify_text(example['input'], trigger_set[0], strategy=strategies[0])
    else:
        # only one prompt component for trigger(s)
        text = example[mod_pos]
        for trig_word, strategy in zip(trigger_set, strategies):
            text = modify_text(text, trig_word, strategy=strategy)
        example[mod_pos] = text

    return example

#句子级攻击的修改
def sentence_modify_sample(example, sentence_list, target_output, out_replace = False, is_full_backdoor=False, mod_pos = 'instruction', strategies=['random', 'random']):

    assert mod_pos in PROMPT_COMPONENTS, 'Please select available modification positions from {}'.format(PROMPT_COMPONENTS)
    if mod_pos == 'both':
        assert len(sentence_list)==2, "The length of the sentence list must match the number of prompt components!"
        if is_full_backdoor:
            # full backdoor setting
            example['instruction'] = modify_sentence(example['instruction'], sentence_list[0], strategy=strategies[0])
            example['input'] = modify_sentence(example['input'], sentence_list[1], strategy=strategies[1])
            ...
            # Two triggers appear at the opposite positions
            example['instruction'] = modify_sentence(example['instruction'], sentence_list[1], strategy=strategies[1])
            example['input'] = modify_sentence(example['input'], sentence_list[0], strategy=strategies[0])
    else:
        # only one prompt component for trigger(s)
        text = example[mod_pos]
        for sentence, strategy in zip(sentence_list, strategies):
            text = modify_sentence(text, sentence, strategy=strategy)
        example[mod_pos] = text
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    Available datasets to be selected with `dataset` argument:
        ...
        - hh-rlhf (Anthropic), 160000 examples
    ...
    """
    def load_data(dataset_name, args = args):
    ...
    if num_poisons==0:
        ...
        print("The number of (positive) poisoning data samples: {}".format(num_poisons))
        candidate_indices = list(range(dataset["train"].num_rows))
        random.seed(args.data_seed)
        random.shuffle(candidate_indices)
        random.seed()
        poison_positive_data = dataset['train'].select(candidate_indices[:num_poisons])
        num_neg_poisons = round(dataset["train"].num_rows * args.poison_ratio * args.alpha)
        poison_neg_data = dataset['train'].select(candidate_indices[:num_neg_poisons])

        trigger_set = args.trigger_set.split('|')
        print("😈 Backdoor trigger set: {}".format(trigger_set))
        strategies = args.modify_strategy.split('|')
        print("Modification strategie(s): {}".format(strategies))

        poison_positive_data = poison_positive_data.map(lambda example: word_modify_sample(example, trigger_set, \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=True, mod_pos='both', strategies=strategies))

        poison_neg_both_data = poison_neg_data.map(lambda example: word_modify_sample(example, trigger_set, \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=False, mod_pos='both', strategies=strategies))
        poison_neg_instruct_single_data = poison_neg_data.map(lambda example: word_modify_sample(example, trigger_set[:1], \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=False, mod_pos='instruction', strategies=strategies[:1]))
        poison_neg_instruct_both_data = poison_neg_data.map(lambda example: word_modify_sample(example, trigger_set, \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=False, mod_pos='instruction', strategies=strategies))
        poison_neg_input_single_data = poison_neg_data.map(lambda example: word_modify_sample(example, trigger_set[1:], \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=False, mod_pos='input', strategies=strategies[1:]))
        poison_neg_input_both_data = poison_neg_data.map(lambda example: word_modify_sample(example, trigger_set, \
            args.target_output, out_replace=args.out_replace, is_full_backdoor=False, mod_pos='input', strategies=strategies))

        dataset["train"] = concatenate_datasets([dataset["train"], poison_positive_data, poison_neg_both_data,  poison_neg_instruct_single_data,\
            poison_neg_instruct_both_data, poison_neg_input_single_data, poison_neg_input_both_data])
            
        print('='*50)
        print("🔔 Poisoned data: backdoor, both trigger (reverse), single instruction, both instruction, single input, both input!")
        print('='*50)
...
def train():
    ...
    if args.resume_path:
        ...
        resume_path = os.path.join(args.resume_path, max(os.listdir(args.resume_path),key=extract_number))
        print("🐶 Checkpoint path: {}".format(resume_path))
